...
CODE_SAMPLE
    ...
    private function processGroupNamesBySuffix(
        ...
        foreach ($groupNamesBySuffix as $singleGroupNamesBySuffix) {
            // has class suffix
            $suffixPattern = '\w+' . $singleGroupNamesBySuffix . '(Test)?\.php$';
            if (! Strings::match($smartFileInfo->getRealPath(), '#' . $suffixPattern . '#')) {
                continue;
            }

            if ($this->isLocatedInExpectedLocation($singleGroupNamesBySuffix, $suffixPattern, $smartFileInfo)) {
                continue;
            }

            // file is already in the group
            if (Strings::match($smartFileInfo->getPath(), '#' . $singleGroupNamesBySuffix . '$#')) {
                continue;
            }

            $this->moveFileToGroupName($smartFileInfo, $fileNode, $singleGroupNamesBySuffix);
            return;
        }
    }
